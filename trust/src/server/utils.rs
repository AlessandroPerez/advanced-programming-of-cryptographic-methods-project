...
pub fn parse_key_array(value: &serde_json::Value) -> Result<[u8; 32], Rejection> {
    let array = value
        .as_array()
        .ok_or_else(|| reject::custom(DeserializationError))?;
    ...
    for (i, v) in array.iter().enumerate() {
        result[i] = v.as_u64().ok_or_else(|| reject::custom(DeserializationError))? as u8;
    }
    Ok(result)
}

/// Helper function to parse a `[u8; 64]` from a JSON array
pub fn parse_key_array64(value: &serde_json::Value) -> Result<[u8; 64], Rejection> {
    let array = value
        .as_array()
        .ok_or_else(|| reject::custom(DeserializationError))?;
    ...
    for (i, v) in array.iter().enumerate() {
        result[i] = v.as_u64()
            .ok_or_else(|| reject::custom(DeserializationError))? as u8;
    }
    Ok(result)
}
